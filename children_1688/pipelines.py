...
class secondIndexupdatePipelines(object):
    def __init__(self):
        ...
        self.writer = csv.writer(self.f,delimiter='\t')
        Logger('all.log', level='debug').logger.info('正在更新Spider , 更新名称 : secondIndex_update 1688我是采购商网站行业大盘二级目录全年指数数据')
    ...
    def close_spider(self, spider):  # 关闭
        Logger('all.log', level='debug').logger.info('更新Spider完成 , 更新数据名称 : secondIndex_update 1688我是采购商网站行业大盘二级目录全年指数数据')
        self.f.close()
...
class secondIndexupdateSupplyPipelines(object):
    ...
    def close_spider(self, spider):  # 关闭
        Logger('all.log', level='debug').logger.info('更新完成 , 更新名称 : secondIndexupdate_supply 1688我是供应商网站行业大盘二级目录全年指数数据')
        self.f.close()
...
class IndustryMarketDownPipelines(object):
    ...
    def process_item(self, item, spider):
        list = [item['category1'],item['category2'],item['industry_Type'],item['industry_Name'],str(item['purchaseIndex1688']),str(item['supplyIndex']),item['demand_Forecast'],item['crawl_Time']]
        self.writer.writerow(list)
        return item
...
class AttributesegmentationMiddlePipelines:
    ...
    def process_item(self, item, spider):
        list = [item['category1'],item['category2'],item['industry_Type'],item['attribute_Name'],str(item['purchaseIndex']),str(item['supplyIndex']),item['crawl_Time']]
        self.writer.writerow(list)
        return item
...
class AttributeSegmentationPricePipelines:
    ...
    def process_item(self, item, spider):
        list = [item['category1'],item['category2'],item['attribute_Type'],item['attribute_Name'],item['index_Type'],str(item['percentage']),item['attribute_Name1'],item['index_Type1'],str(item['percentage1']),item['crawl_Time']]
        self.writer.writerow(list)
        # return item
...
class aLiSupplyFileMarket_pipelines:
    ...
    def close_spider(self,spider):
        Logger('all.log', level='debug').logger.info('更新Spider完成 , 更新数据名称 : alisupplyfilemarket')
        self.f.close()
...
class aLiSupplyFileProduct_pipelines:
    def __init__(self):
        ...
        self.writer = csv.writer(self.f,delimiter='\t')
        Logger('all.log', level='debug').logger.info('正在更新Spider , 更新数据名称: alisupplyfilproduct')
        self.writer.writerow(
            ['公司名','产品', '爬取时间'])
