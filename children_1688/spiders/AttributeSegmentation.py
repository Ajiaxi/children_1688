...
class AttributesegmentationSpider(scrapy.Spider):
    ...
    def parse(self, response):
        # if self.parseApplicableGender(response):
        items = []
        items1 = []
        for i in range(0,32):
        # 适用性别
        ...
        print(items1)
        'debug'
        'http://www.scrapyd.cn/doc/144.html'
        # 是否连帽
        ...
        # industry_Type = response.xpath('//span[@class="ms-yh"]/text()').extract()
        # # 适用性别等
        # attribute_Name1 = response.xpath('//ul[@class="tab-header fd-clr"]/li/text()').extract()
        # # 中性 女性 男性等
        ...
        # purchaseIndex = []
        # supplyIndex = []
        # for i in range(0,len(purchase_supply1),2):
        #     purchaseIndex.append(purchase_supply1[i])
        #     supplyIndex.append(purchase_supply1[i+1])
        # for i in range(0,len(attribute_Name)):
        #     item = AttributesegmentationItem()
        #     item['category1'] = category1
        #     item['category2'] = category2[0]
        #     item['industry_Type'] = industry_Type[0]
        #     item['attribute_Type'] = attribute_Name1[0]
        #     item['attribute_Name'] = attribute_Name[i]
        #     item['purchaseIndex'] = purchaseIndex[i]
        #     item['supplyIndex'] = supplyIndex[i]
        #     item['crawl_Time'] = crawl_Time
