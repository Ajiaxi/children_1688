...
'''
    爬取属性细分热门基础属性
    该spider为所有二级目录的第3个类别 (比如二级目录防晒衣的颜色)
    该spider是文件中5种数据的第3种数据，所以在children_1688.pipelines.AttributesegmentationPipelines中先将写入方式改为a+，并且将字段名写入代码注释

'''

class Attributesegmentation3Spider(scrapy.Spider):
    name = 'AttributeSegmentation3'
