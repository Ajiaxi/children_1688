...
class IndustrymarketdownSpider(scrapy.Spider):
    ...
    custom_settings = {
        'ITEM_PIPELINES' : {'children_1688.pipelines.IndustryMarketDowntest': 300,}
    }
    ...
    def parse(self, response):
        ...
        if self.industry_Type[0] == '热门行业':
            ...
            for i in range(0,len(list_supplyIndex)):
                ...
                item['industry_Type'] = self.industry_Type[0]
            ...
            self.urls1.remove(response.url)
            if self.urls1:
                print('正在爬取：' + str(self.urls1[0]))
                items.append(scrapy.Request(url=self.urls1[0], callback=self.parse))
            else:
                self.industry_Type.remove('热门行业')
                scrapy.Request(url=self.urls2[0], callback=self.parse)
                yield items
        else:
            industry_Name1 = response.xpath('//*[@id="mod-related"]/div[2]/div[2]/div[1]/div[2]/p[1]/@title').extract()
            purchaseIndex16881 = response.xpath(
                '//*[@id="mod-related"]/div[2]/div[1]/div[1]/div[2]/div[2]/p/text()').extract()
            supplyIndex1 = response.xpath(
                '//*[@id="mod-related"]/div[2]/div[1]/div[1]/div[2]/div[3]/p/text()').extract()
            demand_Forecast1 = response.xpath(
                '//*[@id="mod-related"]/div[2]/div[2]/div[1]/div[2]/p[2]/text()').extract()
            crawl_Time = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))
            ...
            if self.urls2:
                ...
                items.append(scrapy.Request(url=self.urls2[0], callback=self.parse))
            else:
                yield items
