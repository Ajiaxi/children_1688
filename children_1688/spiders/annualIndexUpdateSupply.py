...
'''
    - 陈航
    - 更新我是供应商三大指数数据
    - 用法：scrapy crawl everyIndex_supply --nolog 将类Children1688SupplyPipeline中的w(写入)改为a+(追击) ，并且注释掉头列名
'''

class annualIndexUpdateSupplySpider(scrapy.Spider):
    ...
    def parse(self, response):
        print('正在更新Spider..... , 更新数据名称 : 1688我是供应商网站行业大盘童装所有全年三大指数')
        ...
        for i in range(0, len(purchaseIndex1688s)):
            ...
            print('everyIndex_supply 正在爬取日期：' + str(list_Count[i]))
            ...
            yield item
        print('更新Spider完成 , 更新数据名称 : everyIndex_supply 1688我是供应商网站行业大盘童装所有全年三大指数')
    ...
    def datalist(self):
        ...
        for x in range(0, len(data_2018)):
            if datetime.datetime.strptime(data_2018[x], '%Y-%m-%d') >= datetime.datetime.strptime(last_Year_Today,
                                                                                                  '%Y-%m-%d'):
