...
class IndustrymarketdownSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for i in range(0,len(divs)):
            ...
            div = divs[i]
            industry_Name3 = div.xpath('./div[1]/p[1]/text()').extract()
            purchaseIndex16883 = div.xpath('./div[1]/div[2]/p/text()').extract()
            supplyIndex3 = div.xpath('./div[1]/div[3]/p/text()').extract()
            industry_Name3 = str(industry_Name3)[2:-2]
            purchaseIndex16883 = str(purchaseIndex16883)[2:-2]
            supplyIndex3= str(supplyIndex3)[2:-2]
            list_industry_Name.append(industry_Name3)
            list_purchaseIndex1688.append(purchaseIndex16883)
            list_supplyIndex.append(supplyIndex3)
        ...
        for i in range(0,len(list_supplyIndex)):
            ...
            # 热门行业和潜力行业改变
            if i > 4 :
                item['industry_Type'] = industry_Type[1]
