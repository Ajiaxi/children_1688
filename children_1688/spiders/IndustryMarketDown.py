...
class IndustrymarketdownSpider(scrapy.Spider):
    ...
    def datadeal(self,category1,category2,industry_Name1,purchaseIndex16881,supplyIndex1,demand_Forecast1,divs,items,industry_Type):
        # 拼接数据
    ...
    def parse(self, response):
        ...
        items = []
        items = self.datadeal(category1,category2,industry_Name1,purchaseIndex16881,supplyIndex1,demand_Forecast1,divs,items,
                      self.industry_Type[0])
        surl = str(response.url)
        count = surl.find(',')
        resurl = surl[count + 1:]
        if resurl == '127424004':
            print('正在爬取Spider..... , 爬取名称 : 1688网站行业大盘下部数据')
    ...
    def parse2(self, response):
        ...
        if self.next2:
            ...
            print('爬取Spider完成..... , 爬取名称 : 1688网站行业大盘下部数据')
        return items
